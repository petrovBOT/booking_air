...
const IDLE_MS = 30000;
// Было 120000 — и это, как выяснилось по замерам cgroup, и есть механизм
// падений. Невытесняемая память контейнера растёт линейно всё время, пока
// страница открыта: 73MB до запуска браузера, 244-273MB на 30-й секунде
// попытки, 297-309MB на 40-й, 340MB (66% лимита) на 52-й — то есть
// 2.4-4.8MB/с. Отсчёт этого потолка начинается ПОСЛЕ загрузки страницы,
// а она стабильно занимает ~20с, так что 120000 означали попытку длиной
// до 140с — расчётно 108-149% лимита, гарантированный OOM всего
// контейнера. Прогон, который находит рейс, спасался только тем, что
// обрывался сам на ~50с по FIRST_MATCH_GRACE_MS (66%, с запасом). А
// прогон, который рейс НЕ находил, шёл до потолка и убивал процесс —
// ровно так выглядел единственный реальный краш (52-я секунда: ответов
// 56, ok 16, совпадений 0 — поиск работал, но цель не находилась).
// 50с даёт попытку до ~70с и 75-83% даже при худшей скорости, то есть
// безопасно само по себе, не полагаясь на breaker ниже. Полезное окно
// при этом сохраняется: совпадения появляются примерно на 30-й секунде
// ожидания, здесь их ждут почти вдвое дольше.
